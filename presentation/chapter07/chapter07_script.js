(() => {
  ...
  const presentationFile = () => block === 'practice' ? 'chapter07_practice_presentation.html' : 'chapter07_theory_presentation.html';
  ...
  function render() {
    ...
    document.title = `${page}/${total} · Chapter 07 ${block === 'practice' ? '실습' : '이론'} 스크립트`;
  }
  ...
  async function switchData(target, newSlide = 0, newStep = 0, { notify = true } = {}) {
    ...
    document.body.dataset.chapter07Block = block;
    slides = await window.loadChapter07Slides(block);
  ...
  async function switchBlock(target) {
    ...
    presentationWindow = window.open(`${target === 'practice' ? 'chapter07_practice_presentation.html' : 'chapter07_theory_presentation.html'}?v=${ASSET_VERSION}#${target}/1/0`, 'chapter07Presentation');
  ...
  document.getElementById('focusSlides').onclick = () => {
    ...
    presentationWindow = window.open(`${presentationFile()}?v=${ASSET_VERSION}#${block}/${slideIndex + 1}/${stepIndex}`, 'chapter07Presentation');
  ...
  function initialize() {
    if (ready || !window.CH7_SLIDES?.length) return;
    slides = window.CH7_SLIDES;
    if (navigation) navigation.prepareSlides(slides, block);
    const initial = parseHash();
    block = initial.block;
    if (block !== document.body.dataset.chapter07Block) {
  ...
  addEventListener('chapter07-slides-ready', initialize, { once: true });
  addEventListener('chapter07-slides-error', () => {
    if (card) card.innerHTML = '<div class="loading">Chapter 07 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });
  initialize();
})();