(() => {
  ...
  function parseSections(text, block) {
    ...
    return matches.map((match, index) => {
      ...
      return {
        k: `CHAPTER 14 · ${block === 'practice' ? 'PRACTICE' : 'THEORY'} · ${String(number).padStart(2, '0')}`,
        l: label,
        t: title,
  ...
  async function loadChapter14Slides(targetBlock) {
    const block = targetBlock === 'practice' ? 'practice' : 'theory';
    const source = block === 'practice' ? 'chapter14_practice_lecture_plan.md' : 'chapter14_theory_lecture_plan.md';
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Chapter 14 강의 계획서 로드 실패: ${response.status}`);
    const text = await response.text();
    const slides = parseSections(text, block);
    window.CH14_BLOCK = block;
    window.CH14_TITLE = block === 'practice' ? 'Chapter 14 실습 강의' : 'Chapter 14 이론 강의';
    window.CH14_SLIDES = slides;
    dispatchEvent(new CustomEvent('chapter14-slides-ready', { detail: { block, slides } }));
    return slides;
  }

  window.loadChapter14Slides = loadChapter14Slides;
  const initialBlock = document.body?.dataset?.chapter14Block === 'practice' ? 'practice' : 'theory';
  loadChapter14Slides(initialBlock).catch((error) => {
    console.error(error);
    dispatchEvent(new CustomEvent('chapter14-slides-error', { detail: error }));
  });
})();