(() => {
  ...
  function focusTargets(root) {
    ...
    const selectors = [
      ...
      '.example', '.screen-text', '.analysis-flow > *', '.sql-flow > *', '.python-flow > *',
      '.validation-flow > *', '.dataset-flow > *', '.manifest-flow > *', '.chart-flow > *',
      '.question-id', '.period', '.grain', '.metric', '.count-rule', '.date-spine', '.view',
      '.manifest', '.snapshot', '.data-quality', '.cross-check', '.observation', '.interpretation',
      '.limitation', '.read-only', '.hash', '.chart', '.pandas', '.sql-result',
      '.pill', '.chip', 'article'
    ];
  ...
  function render() {
    ...
    document.title = `${slideIndex + 1}/${slides.length} · ${window.CH14_TITLE || `Chapter 14 ${blockLabel} 강의`}`;
  }
  ...
  function openScript() {
    const url = `chapter14_script.html?block=${block}#${block}/${slideIndex + 1}/${stepIndex}`;
    if (scriptWindow && !scriptWindow.closed) {
      scriptWindow.focus();
      sendStateToScript();
      return;
    }
    scriptWindow = window.open(url, 'chapter14Script', 'width=900,height=960,resizable=yes,scrollbars=yes');
    if (scriptWindow) {
      scriptWindow.focus();
  ...
  window.chapter14NextFromScript = next;
  window.chapter14PrevFromScript = previous;
  ...
  addEventListener('chapter14-slides-ready', boot, { once: true });
  addEventListener('chapter14-slides-error', () => {
    app.innerHTML = '<div class="loading-slide error-slide">Chapter 14 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });

  app.innerHTML = '<div class="loading-slide">Chapter 14 발표자료를 구성하는 중입니다.</div>';
  boot();
})();