(() => {
  ...
  const block = document.body?.dataset?.chapter05Block === 'practice' ? 'practice' : 'theory';
  const blockLabel = block === 'practice' ? '실습' : '이론';
  const slides = window.CH5_SLIDES || [];
  const deckTitle = window.CH5_TITLE || `Chapter 05 ${blockLabel} 강의`;
  ...
  function focusTargets(root) {
    ...
    const selectors = [
      '.card', '.flow-step', '.road-step', '.path-node', '.relation-node',
      '.bullet-list li', '.table-wrap tbody tr', '.quote', '.prompt-box',
      'pre', '.codebox', '.expect', '.note', '.callout', '.step-box',
      '.entity', '.attribute', '.relationship', '.rule', '.assumption',
      '.question', '.decision', '.mapping', '.legend', '.scenario',
      '.example', '.result', '.hierarchy > div', '.erd-list > div',
      '.trace-list > div', '.checklist li', '.pill', '.chip', 'article'
    ];
  ...
  function openScript() {
    const url = `chapter05_script.html?block=${block}#${block}/${slideIndex + 1}/${stepIndex}`;
    if (scriptWindow && !scriptWindow.closed) {
      scriptWindow.focus();
      sendStateToScript();
      return;
    }
    scriptWindow = window.open(url, 'chapter05Script', 'width=900,height=960,resizable=yes,scrollbars=yes');
    if (scriptWindow) {
      scriptWindow.focus();
  ...
  window.chapter05NextFromScript = next;
  window.chapter05PrevFromScript = previous;
