(() => {
  ...
  const SCRIPT_FIXES = {
    theory: {
      ...
      'OPTIONALITY': `카디널리티는 최대 연결 개수를, 옵셔널리티는 최소 연결 개수를 나타냅니다.\n\n영에서 하나는 연결 대상이 없어도 되고 생기더라도 하나까지만 허용합니다. 정확히 일은 반드시 하나와 연결됩니다.\n\n영에서 엔은 아직 하나도 없을 수 있지만 시간이 지나면서 여러 개가 생길 수 있습니다. 일에서 엔은 최소 하나가 필요하고 그 이후 여러 개까지 허용합니다. 회원은 대여 이력이 없을 수 있으므로 이번 모델에서는 회원에서 대여 기록을 영에서 엔으로 봅니다.`
    },
    practice: {
      'STEP 04': `회원 이메일의 고유성은 아직 정해지지 않았습니다. 아이에스비엔도 같은 상황입니다.\n\n같은 아이에스비엔의 실제 복본을 각각 관리할지, 한 대여 대상에 미반납 기록을 하나만 허용할지도 추가 결정이 필요합니다.\n\n연체와 삭제 정책 역시 현재 요구사항만으로는 알 수 없습니다. 이 항목들은 제약조건으로 만들지 않고 미확정 질문이나 범위 제외로 남깁니다.`
    }
  };
