(() => {
  ...
  function focusTargets(root) {
    ...
    const selectors = [
      ...
      '.example','.screen-text','.project-flow > *','.evidence-flow > *',
      '.validation-flow > *','.restore-flow > *','.analysis-flow > *','.approval-flow > *',
      '.requirement','.policy','.metadata','.transaction','.counterexample','.performance',
      '.security','.backup','.restore','.dataset','.completion-gate','.diff','.approval',
      '.pill','.chip','article'
    ];
  ...
  function render() {
    ...
    prepareFocus(); applyFocus(); updateButtons();
    history.replaceState(null, '', `#${block}/${slideIndex + 1}/${stepIndex}`);
    document.title = `${slideIndex + 1}/${slides.length} · ${window.CH15_TITLE || `Chapter 15 ${blockLabel} 강의`}`;
  }
  ...
  function openScript() {
    const url = `chapter15_script.html?block=${block}#${block}/${slideIndex + 1}/${stepIndex}`;
    if (scriptWindow && !scriptWindow.closed) { scriptWindow.focus(); sendStateToScript(); return; }
    scriptWindow = window.open(url, 'chapter15Script', 'width=900,height=960,resizable=yes,scrollbars=yes');
  ...
  window.chapter15NextFromScript = next;
  window.chapter15PrevFromScript = previous;
  ...
  addEventListener('chapter15-slides-ready', boot, { once: true });
  addEventListener('chapter15-slides-error', () => { app.innerHTML = '<div class="loading-slide error-slide">Chapter 15 강의 계획서를 불러오지 못했습니다.</div>'; }, { once: true });
  app.innerHTML = '<div class="loading-slide">Chapter 15 발표자료를 구성하는 중입니다.</div>';
  boot();
})();
