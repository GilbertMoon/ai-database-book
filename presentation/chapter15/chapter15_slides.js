(() => {
  ...
  function parseSections(text, block) {
    ...
    return matches.map((match, index) => {
      ...
      return {
        k: `CHAPTER 15 · ${block === 'practice' ? 'PRACTICE' : 'THEORY'} · ${String(number).padStart(2, '0')}`,
        l: label,
        t: title,
  ...
  async function loadChapter15Slides(targetBlock) {
    const block = targetBlock === 'practice' ? 'practice' : 'theory';
    const source = block === 'practice' ? 'chapter15_practice_lecture_plan.md' : 'chapter15_theory_lecture_plan.md';
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Chapter 15 강의 계획서 로드 실패: ${response.status}`);
    const text = await response.text();
    const slides = parseSections(text, block);
    window.CH15_BLOCK = block;
    window.CH15_TITLE = block === 'practice' ? 'Chapter 15 실습 강의' : 'Chapter 15 이론 강의';
    window.CH15_SLIDES = slides;
    dispatchEvent(new CustomEvent('chapter15-slides-ready', { detail: { block, slides } }));
    return slides;
  }

  window.loadChapter15Slides = loadChapter15Slides;
  const initialBlock = document.body?.dataset?.chapter15Block === 'practice' ? 'practice' : 'theory';
  loadChapter15Slides(initialBlock).catch((error) => {
    console.error(error);
    dispatchEvent(new CustomEvent('chapter15-slides-error', { detail: error }));
  });
})();
