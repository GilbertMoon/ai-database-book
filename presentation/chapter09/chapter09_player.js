(() => {
  ...
  function openScript() {
    const url = `chapter09_script.html?block=${block}&v=${ASSET_VERSION}#${block}/${slideIndex + 1}/${stepIndex}`;
  ...
  function boot() {
    if (booted || !window.CH9_SLIDES?.length) return;
    slides = window.CH9_SLIDES;
    if (navigation) {
      navigation.prepareSlides(slides, block);
      const issues = navigation.audit(slides, block);
      if (issues.length) console.warn('[Chapter09] navigation audit', issues);
    }
    booted = true;
