(() => {
  ...
  function focusTargets(root) {
    ...
    const selectors = [
      ...
      '.example', '.screen-text', '.plan-flow > *', '.index-flow > *', '.compare-flow > *',
      '.scan-flow > *', '.validation-flow > *', '.index-candidate', '.plan-node', '.metric',
      '.buffer', '.estimate', '.actual', '.cost', '.scan', '.sort', '.filter', '.index-cond',
      '.pill', '.chip', 'article'
    ];
  ...
  function openScript() {
    const url = `chapter10_script.html?block=${block}#${block}/${slideIndex + 1}/${stepIndex}`;
