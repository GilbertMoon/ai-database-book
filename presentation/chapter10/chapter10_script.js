(() => {
  ...
  const presentationFile = () => block === 'practice' ? 'chapter10_practice_presentation.html' : 'chapter10_theory_presentation.html';
  ...
  function render() {
    ...
    document.title = `${page}/${total} · Chapter 10 ${block === 'practice' ? '실습' : '이론'} 스크립트`;
  }
  ...
  async function switchBlock(target) {
    ...
    presentationWindow = window.open(`${target === 'practice' ? 'chapter10_practice_presentation.html' : 'chapter10_theory_presentation.html'}?v=${ASSET_VERSION}#${target}/1/0`, 'chapter10Presentation');
  ...
  document.getElementById('focusSlides').onclick = () => {
    ...
    presentationWindow = window.open(`${presentationFile()}?v=${ASSET_VERSION}#${block}/${slideIndex + 1}/${stepIndex}`, 'chapter10Presentation');
  ...
  function initialize() {
    ...
    block = initial.block;
    if (block !== document.body.dataset.chapter10Block) {
  ...
  addEventListener('chapter10-slides-error', () => {
    if (card) card.innerHTML = '<div class="loading">Chapter 10 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });
  initialize();
})();
