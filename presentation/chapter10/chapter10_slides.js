(() => {
  ...
  function parseSections(text, block) {
    ...
    return matches.map((match, index) => {
      ...
      return {
        k: `CHAPTER 10 · ${block === 'practice' ? 'PRACTICE' : 'THEORY'} · ${String(number).padStart(2, '0')}`,
        l: label,
        t: title,
  ...
  async function loadChapter10Slides(targetBlock) {
    const block = targetBlock === 'practice' ? 'practice' : 'theory';
    const source = block === 'practice' ? 'chapter10_practice_lecture_plan.md' : 'chapter10_theory_lecture_plan.md';
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Chapter 10 강의 계획서 로드 실패: ${response.status}`);
    const text = await response.text();
    const slides = parseSections(text, block);
    window.CH10_BLOCK = block;
    window.CH10_TITLE = block === 'practice' ? 'Chapter 10 실습 강의' : 'Chapter 10 이론 강의';
    window.CH10_SLIDES = slides;
    dispatchEvent(new CustomEvent('chapter10-slides-ready', { detail: { block, slides } }));
    return slides;
  }

  window.loadChapter10Slides = loadChapter10Slides;
  const initialBlock = document.body?.dataset?.chapter10Block === 'practice' ? 'practice' : 'theory';
  loadChapter10Slides(initialBlock).catch((error) => {
    console.error(error);
    dispatchEvent(new CustomEvent('chapter10-slides-error', { detail: error }));
  });
})();