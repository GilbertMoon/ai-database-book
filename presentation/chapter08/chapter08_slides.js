(() => {
  ...
  function parseSections(text, block) {
    ...
    return matches.map((match, index) => {
      ...
      return {
        k: `CHAPTER 08 · ${block === 'practice' ? 'PRACTICE' : 'THEORY'} · ${String(number).padStart(2, '0')}`,
        l: label,
        t: title,
  ...
  async function loadChapter08Slides(targetBlock) {
    const block = targetBlock === 'practice' ? 'practice' : 'theory';
    const source = block === 'practice' ? 'chapter08_practice_lecture_plan.md' : 'chapter08_theory_lecture_plan.md';
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Chapter 08 강의 계획서 로드 실패: ${response.status}`);
    const text = await response.text();
    const slides = parseSections(text, block);
    window.CH8_BLOCK = block;
    window.CH8_TITLE = block === 'practice' ? 'Chapter 08 실습 강의' : 'Chapter 08 이론 강의';
    window.CH8_SLIDES = slides;
    dispatchEvent(new CustomEvent('chapter08-slides-ready', { detail: { block, slides } }));
    return slides;
  }

  window.loadChapter08Slides = loadChapter08Slides;
  const initialBlock = document.body?.dataset?.chapter08Block === 'practice' ? 'practice' : 'theory';
  loadChapter08Slides(initialBlock).catch((error) => {
    console.error(error);
    dispatchEvent(new CustomEvent('chapter08-slides-error', { detail: error }));
  });
})();
