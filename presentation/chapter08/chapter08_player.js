(() => {
  ...
  function render() {
    ...
    document.title = `${slideIndex + 1}/${slides.length} · ${window.CH8_TITLE || `Chapter 08 ${blockLabel} 강의`}`;
  }
  ...
  function openScript() {
    const url = `chapter08_script.html?block=${block}#${block}/${slideIndex + 1}/${stepIndex}`;
  ...
  window.chapter08NextFromScript = next;
  window.chapter08PrevFromScript = previous;
  ...
  addEventListener('chapter08-slides-error', () => {
    app.innerHTML = '<div class="loading-slide error-slide">Chapter 08 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });

  app.innerHTML = '<div class="loading-slide">Chapter 08 발표자료를 구성하는 중입니다.</div>';
  boot();
})();
