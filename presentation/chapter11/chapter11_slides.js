(() => {
  ...
  function parseSections(text, block) {
    ...
    return matches.map((match, index) => {
      ...
      return {
        k: `CHAPTER 11 · ${block === 'practice' ? 'PRACTICE' : 'THEORY'} · ${String(number).padStart(2, '0')}`,
        l: label,
        t: title,
  ...
  async function loadChapter11Slides(targetBlock) {
    const block = targetBlock === 'practice' ? 'practice' : 'theory';
    const source = block === 'practice' ? 'chapter11_practice_lecture_plan.md' : 'chapter11_theory_lecture_plan.md';
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Chapter 11 강의 계획서 로드 실패: ${response.status}`);
    const text = await response.text();
    const slides = parseSections(text, block);
    window.CH11_BLOCK = block;
    window.CH11_TITLE = block === 'practice' ? 'Chapter 11 실습 강의' : 'Chapter 11 이론 강의';
    window.CH11_SLIDES = slides;
    dispatchEvent(new CustomEvent('chapter11-slides-ready', { detail: { block, slides } }));
    return slides;
  }

  window.loadChapter11Slides = loadChapter11Slides;
  const initialBlock = document.body?.dataset?.chapter11Block === 'practice' ? 'practice' : 'theory';
  loadChapter11Slides(initialBlock).catch((error) => {
    console.error(error);
    dispatchEvent(new CustomEvent('chapter11-slides-error', { detail: error }));
  });
})();