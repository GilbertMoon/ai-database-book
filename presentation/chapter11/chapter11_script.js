(() => {
  ...
  const presentationFile = () => block === 'practice' ? 'chapter11_practice_presentation.html' : 'chapter11_theory_presentation.html';
  ...
  function render() {
    ...
    document.title = `${page}/${total} · Chapter 11 ${block === 'practice' ? '실습' : '이론'} 스크립트`;
  }
  ...
  async function switchBlock(target) {
    ...
    presentationWindow = window.open(`${target === 'practice' ? 'chapter11_practice_presentation.html' : 'chapter11_theory_presentation.html'}?v=${ASSET_VERSION}#${target}/1/0`, 'chapter11Presentation');
  ...
  document.getElementById('focusSlides').onclick = () => {
    ...
    presentationWindow = window.open(`${presentationFile()}?v=${ASSET_VERSION}#${block}/${slideIndex + 1}/${stepIndex}`, 'chapter11Presentation');
  ...
  function initialize() {
    ...
    block = initial.block;
    if (block !== document.body.dataset.chapter11Block) {
  ...
  addEventListener('chapter11-slides-error', () => {
    if (card) card.innerHTML = '<div class="loading">Chapter 11 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });
  initialize();
})();
