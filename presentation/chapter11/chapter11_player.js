(() => {
  ...
  function render() {
    ...
    document.title = `${slideIndex + 1}/${slides.length} · ${window.CH11_TITLE || `Chapter 11 ${blockLabel} 강의`}`;
  }
  ...
  function openScript() {
    ...
    scriptWindow = window.open(url, 'chapter11Script', 'width=900,height=960,resizable=yes,scrollbars=yes');
    if (scriptWindow) {
      scriptWindow.focus();
  ...
  window.chapter11NextFromScript = next;
  window.chapter11PrevFromScript = previous;
