(() => {
  ...
  const navigation = window.CH12Navigation;
  ...
  function openScript() {
    const url = `chapter12_script.html?block=${block}&v=${ASSET_VERSION}#${block}/${slideIndex + 1}/${stepIndex}`;
  ...
  function boot() {
    if (booted || !window.CH12_SLIDES?.length) return;
    slides = window.CH12_SLIDES;
    if (navigation) {
      navigation.prepareSlides(slides, block);
      const issues = navigation.audit(slides, block);
      if (issues.length) console.warn('[Chapter12] navigation audit', issues);
    }
    booted = true;
