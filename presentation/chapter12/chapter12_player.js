(() => {
  ...
  const block = document.body?.dataset?.chapter12Block === 'practice' ? 'practice' : 'theory';
  ...
  function focusTargets(root) {
    ...
    const selectors = [
      ...
      '.example', '.screen-text', '.storage-flow > *', '.cache-flow > *',
      '.document-flow > *', '.event-flow > *', '.graph-flow > *', '.sync-flow > *',
      '.decision-flow > *', '.validation-flow > *', '.source-of-truth', '.derived-data',
      '.key-value', '.document', '.column-family', '.graph', '.jsonb', '.ttl', '.poc',
      '.consistency', '.partition', '.clustering', '.snapshot', '.rebuild',
      '.pill', '.chip', 'article'
    ];
  ...
  function render() {
    ...
    document.title = `${slideIndex + 1}/${slides.length} · ${window.CH12_TITLE || `Chapter 12 ${blockLabel} 강의`}`;
  }
  ...
  function openScript() {
    const url = `chapter12_script.html?block=${block}#${block}/${slideIndex + 1}/${stepIndex}`;
    if (scriptWindow && !scriptWindow.closed) {
      scriptWindow.focus();
      sendStateToScript();
      return;
    }
    scriptWindow = window.open(url, 'chapter12Script', 'width=900,height=960,resizable=yes,scrollbars=yes');
    if (scriptWindow) {
      scriptWindow.focus();
  ...
  window.chapter12NextFromScript = next;
  window.chapter12PrevFromScript = previous;
  ...
  addEventListener('chapter12-slides-ready', boot, { once: true });
  addEventListener('chapter12-slides-error', () => {
    app.innerHTML = '<div class="loading-slide error-slide">Chapter 12 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });

  app.innerHTML = '<div class="loading-slide">Chapter 12 발표자료를 구성하는 중입니다.</div>';
  boot();
})();
