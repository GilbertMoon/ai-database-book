...
포스트그레스큐엘 안의 `nosql_lab` 스키마를 사용해 저장 모델을 판단하는 기준을 연습합니다. 핵심은 “어떤 제품을 쓸 것인가”가 아니라 “이 데이터의 역할과 반복 조회 패턴이 무엇인가”입니다.

실습은 `nosql_lab`만 생성하고 사용합니다. 앞 장의 `코스 프로젝트`, `트랜잭션 랩`, `performance_lab`, `security_lab`은 변경하지 않습니다.
...
기대 데이터베이스는 `ai_database_book`입니다. 현재 스키마가 `nosql_lab`일 필요는 없습니다. 에스큐엘 파일은 `nosql_lab.course_documents`처럼 스키마 이름을 명시합니다.

또 챕터 07의 원본 기준값이 맞는지 확인합니다. 원본이 달라지면 문서 매핑과 검증 결과도 달라질 수 있습니다.
...
이번 실습의 테이블은 실제 노에스큐엘 제품을 그대로 구현한 것이 아닙니다. 개념을 비교하기 위한 포스트그레스큐엘 모델입니다.

`course_documents`는 Document 모델과 제이슨비를 이해하기 위한 테이블입니다. `key_value_cache_examples`는 Key-Value 캐시의 키, 값, 만료 시각을 기록합니다.
...
03 파일에서는 `course_documents`를 조회합니다. 여기서 중요한 것은 어떤 속성을 일반 컬럼으로 두고, 어떤 속성을 제이슨비로 두었는지 보는 것입니다.

`level`은 모든 강의에서 중요하고 자주 검색할 수 있으므로 일반 컬럼입니다. 반면 태그나 옵션은 문서마다 달라질 수 있으므로 제이슨비에 둡니다.

모든 것을 제이슨비에 넣으면 구조가 유연해 보이지만, 제약조건과 검색, 통계, 조인이 어려워질 수 있습니다.
...
문서 데이터가 원본과 연결되는지 확인합니다. `source_course_id`는 챕터 07의 강의 아이디와 대응합니다.
...
그래서 검증 에스큐엘은 스냅샷의 `source_instructor_id`를 기준으로 원본과 대조합니다.
...
제이슨비를 사용할 때는 연산자 차이를 이해해야 합니다. `->`는 제이슨비 값을 반환하고, `->>`는 text 값을 반환합니다.

깊은 경로를 조회할 때는 `#>` 또는 `#>>`를 사용합니다. 키 존재 여부는 `?`, 특정 구조를 포함하는지는 `@>`로 확인할 수 있습니다.

실습에서는 포스트그레스큐엘 태그를 포함한 문서, online 옵션이 true인 문서 등을 조회하며 연산자의 의미를 확인합니다.
...
제이슨비는 유연하지만 책임이 사라지는 것은 아닙니다. 데이터 구조 규칙을 어디에서 검증할지 정해야 합니다.

핵심 규칙은 DB 제약조건으로 두는 것이 좋습니다. 예를 들어 `level`은 일반 컬럼 체크로 검증합니다.

반면 제이슨 내부의 깊은 구조는 애플리케이션과 검증 에스큐엘이 담당할 수 있습니다. 중요한 것은 검증 책임을 명확히 기록하는 것입니다.
...
03 파일의 변경 실습은 문서 버전을 사용한 낙관적 잠금을 보여 줍니다. 업데이트 조건에 `document_version = 1`이 들어갑니다.

영향 행 수가 1이면 내가 읽은 버전이 아직 유효하다는 뜻입니다. 영향 행 수가 0이면 다른 사용자가 먼저 수정했거나, 제이슨 경로가 예상과 다를 수 있습니다.

실습은 변경 결과를 확인한 뒤 롤백합니다. 그래서 기준 데이터는 다시 `certificate=true`, `document_version=1`로 돌아와야 합니다.
...
`jsonb_set`은 제이슨비 문서 일부를 바꿀 때 사용합니다. 하지만 중간 경로가 예상과 다르면 원하는 위치가 바뀌지 않을 수 있습니다.
...
열-Family는 실제 서버를 설치하지 않고 설계 판단만 기록합니다. 먼저 목표 조회를 문장으로 씁니다.
...
Graph DB를 검토할 때는 관계가 존재한다는 사실만 보지 않습니다. 알디비엠에스도 관계를 다룹니다.

단순한 학생별 수강 강의, 강의별 수강 학생은 조인으로 충분할 수 있습니다.

Graph DB는 여러 단계 관계를 반복적으로 탐색하는 것이 핵심일 때 후보가 됩니다. 워크북에는 어떤 질문이 조인 우선이고, 어떤 질문이 Graph 후보인지 구분해서 적습니다.
...
수강신청 원본은 포스트그레스큐엘 알디비엠에스가 채택된 상태입니다. 하지만 로그인 세션이나 인기 강의 캐시는 Key-Value 후보이며, 아직 PoC가 필요할 수 있습니다.
...
알디비엠에스 변경은 성공했지만 캐시 갱신이 실패할 수 있습니다. 강의 원본은 바뀌었는데 문서 스냅샷이 오래된 상태로 남을 수도 있습니다.
...
06 파일에서는 제이슨비 인덱스 후보를 만듭니다. 진 인덱스는 제이슨비 포함 검색 같은 질의에 대응할 수 있습니다.

표현식 비트리 인덱스는 특정 제이슨 경로를 꺼내 비교하는 질의에 대응할 수 있습니다.
...
원본 매핑, 강사 스냅샷 대조, 제이슨비 구조, 문서 버전, 캐시 기준값, 저장소 선택 근거, 인덱스 정의까지 확인합니다.

검증을 통과하면 `챕터 12 nosql_lab validation passed` 메시지가 나옵니다. 이 메시지가 나오면 챕터 12 실습의 기준 상태가 유지된 것입니다.
...
에이아이에게 저장소 추천을 받을 때는 제품 이름만 받아들이면 안 됩니다. 현재 원본이 무엇인지, 어떤 조회가 반복되는지, 오래된 값을 허용할 수 있는지까지 함께 검토해야 합니다.

“트래픽이 많으니 모든 테이블을 노에스큐엘로 옮기자”는 제안은 너무 거칩니다. 수강신청 원본, 캐시, 이벤트, 추천 그래프는 서로 역할이 다릅니다.

최종 완료 기준은 명확합니다. `nosql_lab`만 변경했고, 원본 매핑이 맞고, Seed 기준 캐시와 제이슨비 문서 검증이 맞고, 후보 저장소의 근거와 결정 상태를 설명할 수 있어야 합니다.