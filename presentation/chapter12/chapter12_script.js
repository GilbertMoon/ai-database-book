(() => {
  ...
  const presentationFile = () => block === 'practice' ? 'chapter12_practice_presentation.html' : 'chapter12_theory_presentation.html';
  ...
  function render() {
    ...
    document.title = `${page}/${total} · Chapter 12 ${block === 'practice' ? '실습' : '이론'} 스크립트`;
  }
  ...
  async function switchBlock(target) {
    ...
    presentationWindow = window.open(`${target === 'practice' ? 'chapter12_practice_presentation.html' : 'chapter12_theory_presentation.html'}?v=${ASSET_VERSION}#${target}/1/0`, 'chapter12Presentation');
  ...
  document.getElementById('focusSlides').onclick = () => {
    ...
    presentationWindow = window.open(`${presentationFile()}?v=${ASSET_VERSION}#${block}/${slideIndex + 1}/${stepIndex}`, 'chapter12Presentation');
  ...
  function initialize() {
    ...
    block = initial.block;
    if (block !== document.body.dataset.chapter12Block) {
  ...
  addEventListener('chapter12-slides-error', () => {
    if (card) card.innerHTML = '<div class="loading">Chapter 12 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });
  initialize();
})();
