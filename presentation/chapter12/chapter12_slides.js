(() => {
  ...
  function parseSections(text, block) {
    ...
    return matches.map((match, index) => {
      ...
      return {
        k: `CHAPTER 12 · ${block === 'practice' ? 'PRACTICE' : 'THEORY'} · ${String(number).padStart(2, '0')}`,
        l: label,
        t: title,
  ...
  async function loadChapter12Slides(targetBlock) {
    const block = targetBlock === 'practice' ? 'practice' : 'theory';
    const source = block === 'practice' ? 'chapter12_practice_lecture_plan.md' : 'chapter12_theory_lecture_plan.md';
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Chapter 12 강의 계획서 로드 실패: ${response.status}`);
    const text = await response.text();
    const slides = parseSections(text, block);
    window.CH12_BLOCK = block;
    window.CH12_TITLE = block === 'practice' ? 'Chapter 12 실습 강의' : 'Chapter 12 이론 강의';
    window.CH12_SLIDES = slides;
    dispatchEvent(new CustomEvent('chapter12-slides-ready', { detail: { block, slides } }));
    return slides;
  }

  window.loadChapter12Slides = loadChapter12Slides;
  const initialBlock = document.body?.dataset?.chapter12Block === 'practice' ? 'practice' : 'theory';
  loadChapter12Slides(initialBlock).catch((error) => {
    console.error(error);
    dispatchEvent(new CustomEvent('chapter12-slides-error', { detail: error }));
  });
})();
