...
하지만 student_name은 student_id만으로 결정되고, course_name은 course_id만으로 결정됩니다. 따라서 학생 이름은 스튜던츠로, 강의 이름은 코시스로 옮기고 인롤먼츠에는 두 식별자와 성적을 둡니다.
...
<div class="quote" style="margin-top:20px;font-size:26px">이메일 대소문자, 동일 ISBN 복본과 여러 저자 관리는 이번 실습 범위에서 제외합니다.</div>`,
    s: `챕터 05에서는 이메일과 ISBN의 고유성, 삭제 정책과 동시 대여 규칙을 미확정 질문으로 남겼습니다. 챕터 06 실습에서는 제약조건을 구현하기 위해 이 가운데 필요한 정책을 명시적으로 확정합니다.
...
정규화는 어떤 사실을 어느 테이블에 저장할지 정합니다. 저장하려는 값과 관계가 확정된 업무 규칙에 맞는지는 낫 널, 유니크, 포린 키, 체크와 부분 고유 인덱스 같은 별도의 규칙으로 검사해야 합니다.`
  });
...
필수 이름은 낫 널이 검사합니다. 챕터 06에서 확정한 동일 이메일과 ISBN 문자열의 중복 금지는 유니크 제약조건으로 구현합니다. 존재하지 않는 회원이나 도서를 가리키는 관계는 외래키가 차단하고, 날짜의 선후 관계는 체크 제약조건이 검사합니다.
...
<div class="quote" style="margin-top:18px;font-size:24px">규칙 ID 확인 → DDL 검토 → 정상·경계·오류 실행 순서로 검증합니다.</div>`,
    s: `에이아이는 디디엘 초안을 빠르게 만들 수 있지만, 확정하지 않은 정책을 임의로 추가하거나 필요한 규칙을 빠뜨릴 수 있습니다.

예를 들어 email 유니크는 챕터 06의 C-01과 일치하지만, 이메일 대소문자까지 같은 값으로 처리하도록 확장하면 이번 실습 범위를 넘어섭니다. 모든 외래키에 캐스케이드를 붙이면 C-07의 대여 이력 보존 정책과 충돌할 수 있습니다.

날짜 체크와 활성 대여 제한이 빠져 있다면 문법적으로 테이블은 생성되더라도 잘못된 상태를 저장할 수 있습니다.

따라서 에이아이 제안은 먼저 규칙 아이디와 연결하고, 생성된 디디엘을 읽은 뒤 정상값과 경계값은 성공하고 오류값은 정확한 규칙으로 실패하는지 실행해 확인합니다.`
  });
})();
