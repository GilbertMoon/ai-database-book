(() => {
  ...
  const navigation = window.CH6Navigation;
  ...
  function render() {
    ...
    document.title = `${page}/${total} · Chapter 06 ${block === 'practice' ? '실습' : '이론'} 스크립트`;
  }
  ...
  loadBlock(block, { preserve: true }).then(() => {
    ...
    console.error(error);
    if (card) card.innerHTML = '<div class="loading">Chapter 06 스크립트 데이터를 불러오지 못했습니다.</div>';
  });
})();