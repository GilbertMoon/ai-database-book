...
<div class="title-meta"><span class="pill">PostgreSQL</span><span class="pill">제약조건</span><span class="pill">검증 증거</span></div>`,
    s: `이번 실습의 목적은 에스큐엘 파일을 끝까지 실행하는 것이 아닙니다.
...
무결성 테스트 파일에는 성공해야 하는 예제와 의도적으로 실패해야 하는 예제가 포함되어 있습니다. 실패 에스큐엘은 전체 선택 실행하지 않고 한 문장씩 실행합니다.
...
<div class="grid-3" style="margin-top:22px">
  ...
  <article class="card emphasis"><h3>Transaction</h3><p>자동 커밋 상태 확인</p></article>
    ...
    s: `실습 오류의 상당수는 에스큐엘 문법보다 실행 위치에서 발생합니다.

current_database로 연결한 데이터베이스를 확인하고, current_schema와 search_path로 스키마 이름을 생략했을 때 어떤 객체를 찾는지 확인합니다.

이 실습의 주요 객체는 public 스키마에 생성합니다. 다른 스키마가 먼저 검색되면 같은 이름의 테이블을 잘못 조회할 수 있으므로 주요 에스큐엘에는 public을 명시합니다.
...
이상 현상은 에스큐엘 오류가 아닙니다. 명령은 정상적으로 실행되지만 서로 다른 사실의 수명과 변경 이유를 한 행에 섞었기 때문에 데이터의 의미가 손상되는 구조 문제입니다.`
  },
  {
    k: 'NORMALIZATION',
    l: '개선 구조',
...
<div class="quote" style="margin-top:22px;font-size:26px">DDL 한 줄마다 어떤 규칙 ID를 구현하는지 설명합니다.</div>`,
    s: `전체 크리에이트 테이블 문을 한 글자씩 읽지 않습니다. 제약조건이 어떤 오류를 막는지 중심으로 확인합니다.

낫 널은 값이 없는 상태를 막고, btrim을 사용한 체크는 공백만 입력된 문자열을 막습니다. 유니크는 이번 실습에서 확정한 동일 문자열 중복 금지를 구현합니다.

외래키는 입력한 회원 번호와 도서 번호가 실제 부모 테이블에 존재하는지 검사합니다. 온 딜리트 리스트릭트는 연결된 대여 이력이 있을 때 회원이나 도서를 삭제하지 못하게 합니다.
...
이름과 이메일, 가입일이 규칙을 만족하는 회원을 입력하고 리터닝으로 생성된 아이디와 저장된 값을 확인합니다. 시드 데이터에서 아이디를 직접 지정한 뒤 아이덴티티의 다음 값을 조정했으므로 자동 번호도 예상 범위에서 생성되어야 합니다.
...
오류 에스큐엘을 여러 개 동시에 실행하면 첫 번째 오류 이후 나머지 결과를 구분하기 어렵습니다. 특히 수동 커밋 상태에서는 트랜잭션이 실패 상태가 되어 이후 정상 조회도 실행되지 않을 수 있습니다.
...
이 상태에서는 오류를 수정한 에스큐엘이나 단순 조회도 실행되지 않고 커런트 트랜잭션 이즈 어보티드 메시지가 반복될 수 있습니다.

이때 제약조건을 삭제하거나 연결을 다시 시작하는 것이 아니라 롤백으로 실패한 트랜잭션을 취소합니다. 그다음 기준 행 수와 주요 관계를 다시 조회합니다.

롤백은 오류 원인을 해결하는 명령이 아니라 실패한 작업 단위를 되돌려 다음 에스큐엘을 실행할 수 있는 상태로 복구하는 명령입니다.`
  },
  {
