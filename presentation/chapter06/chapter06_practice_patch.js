...
SHOW search_path;</code></pre><div class="grid-3"><article class="card"><h3>Database</h3><p>ai_database_book</p></article><article class="card"><h3>Object</h3><p>public.table_name 사용</p></article><article class="card emphasis"><h3>Transaction</h3><p>Auto-commit 상태 확인</p></article></div>`,
  ...
  },
  {
    ...
    s: `제약조건은 좋은 아이디어를 임의로 추가하는 기능이 아닙니다. 챕터 05에서 미확정이었던 정책 가운데 이번 실습에서 명시적으로 확정한 C-01부터 C-08만 구현합니다. 이메일 대소문자 정규화, 동일 아이에스비엔의 여러 복본, 여러 저자와 과거 대여 기간 전체 중첩은 이번 범위에 포함하지 않습니다.`
  },
  {
    k: 'DDL',
    l: '규칙 적용',
...
-- 다음 테스트는 새 상태에서 실행</code></pre>`,
    s: `자동 커밋에서는 일반적으로 실패한 문장만 거부됩니다. 하지만 수동 커밋이나 비긴 안에서 오류가 발생하면 현재 트랜잭션이 실패 상태가 되어 이후 정상 쿼리도 실행되지 않을 수 있습니다. 이때 제약조건을 지우는 것이 아니라 롤백으로 실패한 트랜잭션을 끝냅니다. 그 후 다음 테스트를 새 상태에서 실행합니다. 트랜잭션의 상세 원리는 챕터 09에서 다룹니다.`
  },
  {
    k: 'FINAL CHECK',
    l: '최종 검증',
