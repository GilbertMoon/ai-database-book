(() => {
  ...
  const block = document.body?.dataset?.chapter13Block === 'practice' ? 'practice' : 'theory';
  ...
  function focusTargets(root) {
    ...
    const selectors = [
      ...
      '.example', '.screen-text', '.ai-flow > *', '.review-flow > *', '.evidence-flow > *',
      '.approval-flow > *', '.validation-flow > *', '.diff-flow > *', '.requirement',
      '.decision-id', '.test-id', '.validation-id', '.metadata', '.constraint', '.boundary',
      '.counterexample', '.sqlstate', '.diff', '.approval', '.risk', '.proof',
      '.pill', '.chip', 'article'
    ];
  ...
  function render() {
    ...
    document.title = `${slideIndex + 1}/${slides.length} · ${window.CH13_TITLE || `Chapter 13 ${blockLabel} 강의`}`;
  }
  ...
  function openScript() {
    const url = `chapter13_script.html?block=${block}#${block}/${slideIndex + 1}/${stepIndex}`;
    if (scriptWindow && !scriptWindow.closed) {
      scriptWindow.focus();
      sendStateToScript();
      return;
    }
    scriptWindow = window.open(url, 'chapter13Script', 'width=900,height=960,resizable=yes,scrollbars=yes');
    if (scriptWindow) {
      scriptWindow.focus();
  ...
  window.chapter13NextFromScript = next;
  window.chapter13PrevFromScript = previous;
  ...
  addEventListener('chapter13-slides-ready', boot, { once: true });
  addEventListener('chapter13-slides-error', () => {
    app.innerHTML = '<div class="loading-slide error-slide">Chapter 13 강의 계획서를 불러오지 못했습니다.</div>';
  }, { once: true });

  app.innerHTML = '<div class="loading-slide">Chapter 13 발표자료를 구성하는 중입니다.</div>';
  boot();
})();