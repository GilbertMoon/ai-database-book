(() => {
  ...
  const navigation = window.CH13Navigation;
  ...
  function openScript() {
    const url = `chapter13_script.html?block=${block}&v=${ASSET_VERSION}#${block}/${slideIndex + 1}/${stepIndex}`;
  ...
  function boot() {
    if (booted || !window.CH13_SLIDES?.length) return;
    slides = window.CH13_SLIDES;
    if (navigation) {
      navigation.prepareSlides(slides, block);
      const issues = navigation.audit(slides, block);
      if (issues.length) console.warn('[Chapter13] navigation audit', issues);
    }
    booted = true;
