(() => {
  ...
  function parseSections(text, block) {
    ...
    return matches.map((match, index) => {
      ...
      return {
        k: `CHAPTER 13 · ${block === 'practice' ? 'PRACTICE' : 'THEORY'} · ${String(number).padStart(2, '0')}`,
        l: label,
        t: title,
  ...
  async function loadChapter13Slides(targetBlock) {
    const block = targetBlock === 'practice' ? 'practice' : 'theory';
    const source = block === 'practice' ? 'chapter13_practice_lecture_plan.md' : 'chapter13_theory_lecture_plan.md';
    const response = await fetch(source, { cache: 'no-store' });
    if (!response.ok) throw new Error(`Chapter 13 강의 계획서 로드 실패: ${response.status}`);
    const text = await response.text();
    const slides = parseSections(text, block);
    window.CH13_BLOCK = block;
    window.CH13_TITLE = block === 'practice' ? 'Chapter 13 실습 강의' : 'Chapter 13 이론 강의';
    window.CH13_SLIDES = slides;
    dispatchEvent(new CustomEvent('chapter13-slides-ready', { detail: { block, slides } }));
    return slides;
  }

  window.loadChapter13Slides = loadChapter13Slides;
  const initialBlock = document.body?.dataset?.chapter13Block === 'practice' ? 'practice' : 'theory';
  loadChapter13Slides(initialBlock).catch((error) => {
    console.error(error);
    dispatchEvent(new CustomEvent('chapter13-slides-error', { detail: error }));
  });
})();