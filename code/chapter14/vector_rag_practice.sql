...
-- VALUES
-- ('과제 제출 안내', '과제는 지정된 파일명으로 마감일까지 제출해야 합니다.', 'course_notice.md', 1, '[0.10, 0.80, 0.20]'),
-- ('기말 프로젝트 안내', '기말 프로젝트는 발표 자료와 구현 결과를 함께 제출합니다.', 'course_notice.md', 2, '[0.20, 0.75, 0.25]'),
-- ('수강 취소와 환불', '수강 취소와 환불은 학교 규정에 따라 처리됩니다.', 'course_notice.md', 3, '[0.90, 0.10, 0.20]'),
-- ('질의응답 안내', '질문은 Slack 또는 이메일로 할 수 있습니다.', 'course_notice.md', 4, '[0.25, 0.25, 0.85]'),
-- ('SQL JOIN 복습', 'JOIN은 여러 테이블의 데이터를 연결해 조회할 때 사용합니다.', 'sql_review.md', 1, '[0.15, 0.30, 0.90]');

-- 질문 예시: "기말 프로젝트는 무엇을 제출해야 하나요?"
...
SELECT
    '기말 프로젝트는 무엇을 제출해야 하나요?' AS user_question,
    0.18,
    0.78,
    0.22,
...
(
    '기말 프로젝트는 무엇을 제출해야 하나요?',
    '기말 프로젝트는 발표 자료, 구현 결과, 출석 증명서, 결제 영수증을 제출해야 합니다.',
    2,
    TRUE,
    FALSE,
    TRUE,
    '출석 증명서와 결제 영수증은 검색 문서에 없는 내용입니다.'
);
