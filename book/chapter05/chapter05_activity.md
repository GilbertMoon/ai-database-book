...
```text
chapter05_modeling_practice_hong.md
```
...
실습 1에서 찾은 명사 중 테이블 후보가 될 엔터티와 컬럼이 될 속성을 구분합니다.
...
```text
loans 테이블에 member_id와 book_id가 필요한 이유를 설명해 봅니다.
```
...
이 관계를 loans 테이블로 어떻게 풀어냈는지 설명해 봅니다.
```

---

## 10. 실습 7: ERD 초안 텍스트로 작성

그림 도구를 사용하지 않아도 됩니다. 먼저 텍스트로 ERD 초안을 작성해 봅니다.
...
## 11. 실습 8: library_schema.sql 실행 결과 기록
...
## 17. 실습 14: ChatGPT로 ERD 초안 만들기

아래 요구사항을 ChatGPT에게 입력할 프롬프트로 바꿔 봅니다.
...
이 실습은 다음 흐름으로 진행하면 요구사항 분석부터 설계 검증까지 자연스럽게 이어집니다.

```text
1. 요구사항 읽기 및 명사 찾기
2. 엔터티/속성 구분
3. 관계 분석 및 ERD 초안 작성
4. library_schema.sql 실행
5. JOIN과 외래키 오류 검증
6. AI 생성 ERD 검토
7. 실습 기록 정리
```
...
이 실습의 핵심은 요구사항을 테이블 구조로 바꾸는 것입니다.
