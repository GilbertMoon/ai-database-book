...
예를 들어 다음과 같은 students 테이블이 있다고 가정해 보겠습니다.
...
이런 구조에서는 SQL이 복잡해지고, 데이터 중복이 많아지고, 수정 실수가 발생하기 쉽습니다.
...
따라서 이 요구사항에서 주요 엔터티는 다음 세 가지입니다.
...
예를 들어 loans 테이블의 `member_id`는 members 테이블의 `id`를 참조합니다.

---

## 5. 기본키와 외래키 설계하기

기본키는 한 테이블 안에서 각 행을 구분하는 컬럼입니다.

외래키는 다른 테이블의 기본키를 참조해 관계를 만드는 컬럼입니다.

도서관 예제에서는 다음처럼 설계할 수 있습니다.
...
loans 테이블은 members와 books를 연결합니다.
...
도서관 예제에서 members와 loans는 1:N 관계입니다.
...
이 경우 students와 courses 사이에 enrollments 테이블을 둡니다.

```text
students 1:N enrollments N:1 courses
```

도서관 예제의 members와 books도 넓게 보면 N:M 관계입니다.
...
이 요구사항에서 테이블 후보를 정리하면 다음과 같습니다.
...
| 테이블 | 속성 |
| --- | --- |
| members | name, email, joined_at |
| books | title, author, published_year, isbn |
| loans | member_id, book_id, borrowed_at, due_at, returned_at |
...
이 구조에서 loans는 단순한 연결 테이블이 아니라 대여일, 반납예정일, 실제반납일을 가진 업무 테이블입니다.

따라서 loans는 중요한 엔터티로 보아야 합니다.
...
이 두 줄이 loans 테이블을 members와 books에 연결합니다.
...
대여 기록을 회원 이름과 책 제목으로 조회하려면 다음 SQL을 사용합니다.
...
이 SQL을 실행했을 때 다음과 같은 결과를 얻을 수 있어야 합니다.
...
AI는 다음과 같은 결과를 줄 수 있습니다.

```text
members, books, loans 테이블을 만들고,
loans에 member_id와 book_id를 두어 members와 books를 연결한다.
```

이 답변은 좋은 출발점입니다. 하지만 그대로 믿으면 안 됩니다.
...
예를 들어 회원과 책의 관계를 단순히 members 테이블에 book_id 컬럼으로 넣으면 문제가 됩니다.

```text
한 회원이 여러 권의 책을 대여할 수 있는데, members.book_id 하나로는 여러 권을 표현하기 어렵다.
```

이 경우 loans 같은 중간 테이블이 필요합니다.
...
1. [기초] 데이터 모델링이 필요한 이유를 설명하세요.
2. [기초] 엔터티와 속성의 차이를 설명하세요.
3. [기초] 기본키와 외래키의 차이를 설명하세요.
4. [기초] 1:N 관계에서 외래키는 보통 어느 쪽 테이블에 들어가나요?
5. [기초] N:M 관계를 테이블로 표현하는 방법을 설명하세요.
...
이번 장에서는 데이터 모델링과 ERD의 기초를 학습했습니다.
...
Chapter 05에서 만든 ERD와 테이블 구조는 Chapter 06 정규화 학습의 출발점이 됩니다.
