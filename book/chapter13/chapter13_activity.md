...
> 용도: 수업 활동지 / 자기주도 실습 과제 / Chapter 13 보조 자료

---

## 1. 활동 개요

이 활동 자료는 ChatGPT와 Codex가 생성한 데이터베이스 설계와 SQL을 사람이 검토하고 보완하는 연습을 위한 자료입니다.
...
```text
학번_이름_chapter13_activity.md
```

예시:

```text
20260001_홍길동_chapter13_activity.md
```

---

## 4. 활동 1: AI 설계 초안 검토 기준 정리

AI가 데이터베이스 설계를 생성했을 때 확인해야 할 항목을 정리하세요.
...
```text
AI가 만든 SQL이 실행된다고 해서 좋은 설계라고 볼 수 없는 이유를 설명하세요.
```

---

## 5. 활동 2: 실습 SQL 실행 전 안전 검토

다음 파일을 실행하기 전에 안전 검토표를 작성하세요.
...
## 6. 활동 3: AI가 만든 나쁜 설계 분석

실습 SQL의 `ai_bad_enrollments` 테이블을 확인하세요.
...
## 10. 활동 7: 좋은 설계 조회 결과 확인

다음 SQL을 실행하고 결과 일부를 기록하세요.
...
다음 중 2개 이상을 선택해 하나씩 주석 해제하고 오류 메시지를 확인하세요.
...
## 12. 활동 9: information_schema로 메타데이터 확인

다음 SQL을 실행해 테이블, 컬럼, 제약조건, 외래키 정보를 확인하세요.
...
## 15. 활동 12: SQL Anti-pattern 찾기

다음 Anti-pattern의 문제와 수정 방향을 작성하세요.
...
## 17. 활동 14: AI 검토 프롬프트 작성

ChatGPT에게 AI가 만든 DDL을 검토시키는 프롬프트를 작성하세요.
...
AI가 만든 DB 설계와 SQL을 검토하면서 알게 된 점을 3~5문장으로 작성하세요.
```
...
| AI/Codex 프롬프트와 제출 형식 | 15 | 오류 수정 루프와 AI 검토 프롬프트를 구체적으로 작성했는가 |
...
```text
1. AI 생성 설계는 초안이라는 관점 설명: 15분
2. 나쁜 설계 테이블 분석: 30분
3. 좋은 설계 SQL 실행 및 비교: 35분
4. 제약조건 오류 확인 실습: 25분
5. information_schema 결과 해석: 25분
6. SQL Anti-pattern 토론: 20분
7. Codex 오류 수정 루프 작성: 20분
8. AI 검토 프롬프트 작성 및 공유: 30분
```

초급자에게는 SQL 작성 자체보다 **AI 결과를 읽고 검토하는 사고 과정**을 강조합니다.

---

## 23. 핵심 정리

이 활동의 핵심은 AI 생성 결과를 검토하는 능력입니다.
