...
Chapter 02에서는 테이블, 행, 열, 기본키, 외래키, 제약조건을 배웠고, Chapter 03에서는 PostgreSQL과 DBeaver 실습 환경을 구축했습니다. 이제부터는 실제 SQL을 작성해 데이터를 생성하고, 조회하고, 수정하고, 삭제하는 기본 흐름을 익힙니다.
...
1. [실습] students 테이블의 모든 데이터를 조회하는 SQL을 작성하세요.
2. [실습] students 테이블에서 name과 email만 조회하는 SQL을 작성하세요.
3. [실습] 전공이 데이터사이언스인 학생만 조회하는 SQL을 작성하세요.
4. [실습] 학년이 2 이상인 학생을 조회하는 SQL을 작성하세요.
5. [실습] 학생을 학년 내림차순으로 정렬하는 SQL을 작성하세요.
...
이번 장에서는 관계형 데이터베이스에서 가장 기본이 되는 SQL을 학습했습니다.
