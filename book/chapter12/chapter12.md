...
이 장에서는 관계형 데이터베이스와 다른 방식으로 데이터를 저장하는 **NoSQL**의 기본 개념과 선택 기준을 학습합니다.

지금까지 이 책에서는 PostgreSQL을 중심으로 관계형 데이터베이스, SQL, 모델링, 정규화, JOIN, 트랜잭션, 인덱스, 보안과 백업을 학습했습니다. 관계형 데이터베이스는 여전히 매우 중요합니다. 특히 주문, 결제, 수강신청, 재고, 회계처럼 정합성이 중요한 데이터에는 관계형 데이터베이스가 강력합니다.
...
1. NoSQL이 무엇인지 설명하세요.
2. 관계형 데이터베이스와 NoSQL의 차이를 3가지 이상 설명하세요.
...
다음 데이터에 적합한 저장 방식을 선택하고 이유를 작성하세요.
...
이 추천의 문제점을 설명하세요. 다음 기준을 포함해야 합니다.
...
이번 장에서는 NoSQL의 기본 개념과 선택 기준을 학습했습니다.
...
다음 장에서는 ChatGPT와 Codex를 활용해 데이터베이스 설계와 SQL을 생성하고 검증하는 방법을 학습합니다.
