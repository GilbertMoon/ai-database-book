...
샘플 데이터는 다음을 포함하는 것이 좋습니다.

```text
- 정상 데이터
- 관계가 연결된 데이터
- 상태값이 다른 데이터
- 집계나 JOIN 결과를 확인할 수 있는 데이터
```
...
선택 확장을 반드시 구현할 필요는 없습니다. 다만 다음 질문에 답할 수 있어야 합니다.

```text
- 왜 이 기능에 해당 기술이 적합한가?
- 관계형 DB만 사용할 때의 한계는 무엇인가?
- 해당 기술을 도입하면 어떤 운영 부담이 생기는가?
- 실제 프로젝트에서는 언제 도입하는 것이 적절한가?
```
