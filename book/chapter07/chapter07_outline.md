...
- 전용 스키마에 프로젝트 구조를 원자적으로 생성한다.
- 검증 목적의 샘플 데이터를 설계한다.
- 예상 이전 상태를 확인하며 데이터를 변경한다.
- 구조·관계·데이터 자동 검증으로 최종 상태를 판정한다.
- 허용 경계와 금지 오류를 실제 제약조건과 연결한다.
...
| P07-D02 | 신청 생성 시 현재 courses.price를 recorded_amount에 복사·보존 | INSERT ... SELECT·NUMERIC(12,0) |
...
| 02 | 네 테이블 비어 있음 | 기본 3/2/3/4행·관계 기준·470000 |
| 03 | 기본 샘플 상태 | 최종 3/2/3/5행·590000·취소 제외 440000 |
| 04 | 최종 상태 | 구조·관계·도메인·상태·금액 자동 검증 통과 |
| 05 | 최종 상태 | 구조 기준 재확인·핵심 요구사항 경계·오류 테스트 |
| 06 | 최종 상태 | 선택 경계·공백 문자열 테스트 |
| reset | 어떤 프로젝트 상태 | 알려진 객체를 원자적으로 삭제 |
...
Chapter 08에서는 최종 신청 5건을 기준으로 JOIN과 집계를 수행한다. 실행 전 `code/chapter08/00_check_course_project.sql`이 Chapter 07 최종 상태와 `recorded_amount` 기준을 검사한다. 기본 검산값은 전체 590000, 활성 신청 340000, 취소 제외 440000이다.
