...
이 장에서는 여러 테이블의 데이터를 함께 조회하는 **JOIN**과 데이터를 요약하는 **집계 쿼리**를 학습합니다.
...
1. INNER JOIN과 LEFT JOIN의 차이를 설명하세요.
2. GROUP BY가 필요한 이유를 설명하세요.
3. WHERE와 HAVING의 차이를 설명하세요.
4. LEFT JOIN에서 COUNT(*) 대신 COUNT(e.id)를 사용할 때의 장점을 설명하세요.
5. AI가 만든 JOIN SQL을 검토할 때 확인해야 할 항목을 3가지 이상 쓰세요.
...
1. 학생별 수강신청 수를 조회하세요.
2. 강의별 총 결제금액을 조회하세요.
3. 수강상태별 수강신청 수를 조회하세요.
4. 강사별 개설 강의 수를 조회하세요.
5. 수강생이 2명 이상인 강의만 조회하세요.
```
...
예를 들어 수강신청과 결제 처리는 함께 성공하거나 함께 실패해야 합니다. 이를 위해 트랜잭션, COMMIT, ROLLBACK, 데이터 정합성 개념을 학습합니다.
