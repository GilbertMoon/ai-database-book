...
# 00: Chapter 07 structural handoff
p = 'code/chapter08/00_check_course_project.sql'
replace_exact(p, '    v_active_duplicate_count bigint;\n    v_status_1001 text;', '    v_active_duplicate_count bigint;\n    v_named_constraint_count bigint;\n    v_not_null_count bigint;\n    v_status_1001 text;')
replace_exact(p, '    SELECT COUNT(*) INTO v_student_count\n    FROM course_project.students;', '''    SELECT COUNT(*) INTO v_named_constraint_count
    FROM pg_constraint
    WHERE conrelid IN (
...
# 03: HAVING and over-aggregation evidence
p = 'code/chapter08/03_join_aggregation_validation.sql'
replace_exact(p, '    v_instructor202_non_cancelled bigint;\nBEGIN', '''    v_instructor202_non_cancelled bigint;
...
BEGIN''')
replace_exact(p, '    IF v_detail_count <> 5', '''    SELECT COUNT(*) INTO v_having_course_count
    FROM (
        SELECT c.id
    ...
    IF v_detail_count <> 5''')
replace_exact(p, "    RAISE NOTICE 'Chapter 08 join and aggregation validation passed';", '''    IF v_having_course_count <> 2
...
replace_exact(p, '-- 10. 여러 JOIN에서 강의 가격을 잘못 합산하는 예', '-- 10. 여러 JOIN에서 강의 가격을 잘못 합산하는 예\n-- 기준 데이터에서 강사 201은 잘못된 합계 440000, 실제 강의 가격 합계 220000입니다.\n-- 강사 202는 신청이 한 건뿐이라 두 값이 우연히 150000으로 같지만, 쿼리 기준이 올바르다는 뜻은 아닙니다.')

# Book numeric evidence
p = 'book/chapter08/chapter08.md'
replace_exact(p, "GROUP BY i.id;\n```\n\n`SUM(DISTINCT c.price)`도 일반적인 해결책이 아닙니다.", '''GROUP BY i.id;
```
...
replace_exact(p, "FROM course_project.courses\nGROUP BY instructor_id\nORDER BY instructor_id;\n```\n\n집계 전에 어떤 테이블의 한 행이 합산 대상인지 정해야 합니다.", '''FROM course_project.courses
GROUP BY instructor_id
ORDER BY instructor_id;
```
...
replace_exact(p, "- 위험: 신청 JOIN 후 `SUM(c.price)`", "- 위험: 신청 JOIN 후 `SUM(c.price)`\n- 강사 201: 잘못된 합계 440000 → 올바른 강의 합계 220000")
replace_exact(p, '무엇을 합산하는지 먼저 정하고, 그 기준 행이 있는 테이블에서 계산해야 합니다.', '현재 기준 데이터에서는 강사 201의 잘못된 합계가 440000이고, 강의 수준에서 계산한 올바른 합계는 220000입니다. 강사 202는 신청이 한 건뿐이라 두 값이 150000으로 우연히 같지만 잘못된 집계 방식이 올바르게 바뀐 것은 아닙니다. 무엇을 합산하는지 먼저 정하고, 그 기준 행이 있는 테이블에서 계산해야 합니다.')

p = 'presentation/chapter08/chapter08_practice_lecture_plan.md'
replace_exact(p, "- 잘못된 합계: 신청 JOIN 뒤 `SUM(c.price)`", "- 잘못된 합계: 신청 JOIN 뒤 `SUM(c.price)`\n- 강사 201: 440000(잘못) / 220000(올바름)")
replace_exact(p, '`썸(디스팅트 c.price)`도 일반 해결책이 아닙니다. 서로 다른 강의가 같은 가격이면 한 번만 더해질 수 있기 때문입니다.', '`썸(디스팅트 c.price)`도 일반 해결책이 아닙니다. 서로 다른 강의가 같은 가격이면 한 번만 더해질 수 있기 때문입니다. 실제로 강사 201은 신청 조인 뒤 440000으로 계산되지만, 강의 301과 302의 기준 가격만 더하면 220000입니다. 강사 202처럼 두 결과가 우연히 같은 경우에도 쿼리의 기준이 올바른 것은 아닙니다.')

# README
p = 'code/chapter08/README.md'
replace_exact(p, 'recorded_amount = NUMERIC(12,0)\n행 수 = 3 / 2 / 3 / 5', 'recorded_amount = NUMERIC(12,0)\nChapter 07 명명 제약조건 = 15 / NOT NULL 열 = 20\n행 수 = 3 / 2 / 3 / 5')
replace_exact(p, '강사 202 = 강의 1 / 신청 1 / 취소 제외 0', '강사 202 = 강의 1 / 신청 1 / 취소 제외 0\nHAVING 취소 제외 2건 이상 강의 = 2개\n강사 201 가격: 신청 JOIN 뒤 잘못된 합계 440000 / 강의 수준 올바른 합계 220000\n강사 202 가격: 두 방식 모두 150000(우연히 같음)')

# Authored narration, no generic expansion
