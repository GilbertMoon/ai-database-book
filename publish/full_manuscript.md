...
오히려 반대입니다. AI가 만드는 결과가 많아질수록 사람은 데이터의 의미와 관계를 이해하고, 생성된 구조와 숫자가 실제 요구사항에 맞는지 판단해야 합니다. SQL이 오류 없이 실행된다는 사실만으로 결과가 올바르다고 볼 수 없으며, 테이블이 만들어졌다는 사실만으로 장기간 운영 가능한 구조라고 판단할 수도 없습니다.

이 장에서는 다음 내용을 살펴봅니다.

- AI가 SQL을 만들어 주어도 데이터베이스를 배워야 하는 이유
- 실행되는 SQL과 요구사항에 맞는 SQL의 차이
- 잘못된 데이터와 조회 기준이 AI 분석에 미치는 영향
- 현실의 업무를 데이터와 관계로 표현하는 방법
- 파일·스프레드시트·DBMS 중심 관리 방식을 선택하는 기준
- 기준 데이터와 파생 데이터, AI 생성 결과를 구분하는 이유
- AI가 잘하는 일과 사람이 확인해야 할 일
- 이 책에서 사용할 AI 활용·실행·검증 흐름
...
> **예제 SQL 안내**
>
> 이 장의 SQL은 개념을 설명하기 위한 축약 예제입니다. 실제 PostgreSQL 데이터베이스와 테이블은 Chapter 03~04에서 생성합니다.
...
이 다섯 가지 관점은 이후 장의 설계, SQL, 트랜잭션, 인덱스, 보안, 분석과 AI 검증 실습을 연결하는 기준이 됩니다.
...
탈퇴 학생이나 비활성 학생을 제외해야 한다면 `status` 같은 업무 속성과 별도의 필터 기준이 필요합니다. 반대로 질문이 없는 학생까지 포함한 **학생별 질문 수**를 계산하려면 학생을 기준으로 질문을 `LEFT JOIN`하고 학생별로 집계해야 합니다. 자세한 방법은 Chapter 08에서 다룹니다.

SQL이 실행되어 숫자가 나왔더라도 다음을 확인해야 합니다.
...
- 학생, 강의, 강사와 신청이라는 성격이 다른 정보가 한 테이블에 섞여 있습니다.
- 필수값과 허용값을 제한하는 규칙이 없습니다.
- 학생의 이메일이나 활동 상태를 안정적으로 관리하기 어렵습니다.
- 진행 중인 중복 신청을 금지해야 해도 이름과 제목만으로는 안정적으로 차단하기 어렵습니다.
- 결제 시도·실패·환불 이력까지 관리하려면 별도의 결제 구조가 필요합니다.

여기서 `recorded_amount`는 신청 당시 기록한 금액일 뿐 실제 결제 완료 금액이나 매출을 자동으로 의미하지 않습니다. 실제 상용 서비스에서 결제 성공, 실패와 환불을 관리하려면 별도의 결제 원장과 상태가 필요합니다.

Chapter 07의 기본 프로젝트는 학생·강사·강의·수강신청 네 테이블과 신청 당시 기록 금액에 집중합니다. 결제 시도·실패·환불은 확장 범위로 분리합니다.
...
| --- | --- | --- | --- |
| 데이터 품질 | 주문이 중복 저장됨 | 금액이 실제보다 크게 계산됨 | AI가 성과가 증가했다고 잘못 설명함 |
...
사용자, 업무 상태와 실행 이력을 지속적으로 관리하는 AI 서비스는 대부분 데이터베이스가 필요합니다. 화면에 보이는 답변 외에도 다음과 같은 데이터를 관리해야 합니다.

| 데이터 영역 | 예시 |
| --- | --- |
| 사용자와 권한 | 사용자 계정, 역할, 접근 범위 |
| 핵심 업무 데이터 | 주문, 예약, 질문, 콘텐츠 |
| 작업 상태 | 요청 대기, 처리 중, 완료, 실패 |
| 실행 이력 | 요청 시간, 처리 결과, 오류 내용 |
| AI 실행 맥락 | 모델 식별자, 입력 데이터 기준 시점, 프롬프트 템플릿 버전, 실행 ID |
| AI 결과 | 요약, 분류, 추천, 생성된 답변 |
| 검토 기록 | 승인 여부, 수정 내용, 검토자, 검토 시각 |
| 보존 정책 | 보존 기간, 민감정보 포함 여부, 삭제 기준 |

AI 실행 맥락은 결과를 재현하고 원인을 추적하는 데 필요한 최소 범위에서 기록합니다. 개인정보, 비밀정보와 전체 프롬프트를 무조건 저장하는 것이 아니라 보안, 감사와 보존 정책을 함께 고려해야 합니다.
...
| 마케터 | 고객, 캠페인과 구매 데이터를 올바르게 해석하기 위해 |
...
이 비교는 저장 매체의 엄격한 분류가 아니라 **일반적인 사용 방식**을 비교한 것입니다. SQLite처럼 파일 하나로 사용하는 관계형 DBMS도 있고, 클라우드 스프레드시트처럼 여러 사람이 동시에 편집하는 도구도 있습니다.
...
| 동시 수정 | 충돌 관리가 별도로 필요 | 공동 편집 가능, 복잡한 업무 규칙 통제에는 한계 | 트랜잭션과 잠금으로 여러 작업 조정 가능 |
| 검색·집계 | 별도 코드가 필요한 경우가 많음 | 필터, 함수와 피벗 사용 | SQL로 반복 조회와 복잡한 집계 가능 |
| 정확성 | 프로그램·사용자가 직접 확인 | 데이터 검증과 수식으로 일부 관리 | 제약조건과 트랜잭션을 올바르게 설계해 일관성 관리 |
| 권한·복구 | 운영체제 파일 권한과 별도 백업 | 공유 권한과 버전 이력 중심 | 역할별 권한, 백업과 복구 절차 구성 가능 |
...
| 구분 | 설명 | 예시 |
| --- | --- | --- |
| 기준 데이터<br>(Source of Truth) | 업무에서 직접 발생하고 판단의 기준이 되는 데이터 | 회원, 주문, 신청, 결제 원장, 출석 원본 |
| 결정적 파생 데이터<br>(Derived Data) | 정해진 계산식으로 기준 데이터를 가공한 결과 | 월별 집계, 대시보드 지표, 캐시 |
| AI 생성 결과 | 모델과 입력 데이터·지시문을 사용해 만든 결과 | 요약, 분류, 추천, 생성 답변 |

결정적 파생 데이터는 같은 기준 데이터, 계산식과 기준 시점이 있으면 다시 만들 수 있는 경우가 많습니다. 기준 데이터가 변경되면 집계와 캐시도 최신 상태인지 확인하고 필요하면 재생성해야 합니다.
...
AI 생성 결과는 조금 다릅니다. 다음 조건이 달라지면 결과도 달라질 수 있습니다.
...
따라서 재현과 감사를 위해 입력 스냅샷 또는 기준 시점, 모델 식별자, 지시문 버전, 실행 시각과 실행 ID를 기록해야 합니다. 그래도 완전히 같은 문장이 다시 생성된다고 보장할 수는 없습니다. 사람이 승인한 AI 결과가 실제 업무 기록이 되었다면 재생성 가능한 임시 결과로만 보지 않고 별도로 보존해야 합니다.

Chapter 01에서는 이 구분이 필요한 이유만 이해하면 충분합니다. 구체적인 정의와 구조는 Chapter 02와 후반 프로젝트에서 다시 살펴봅니다.
...
정적 강의 목록 몇 개만 보여 주는 초기 단계라면 파일도 사용할 수 있습니다. 그러나 회원가입, 신청, 상태 변경, 반복 집계와 여러 운영자의 동시 작업이 시작되면 DBMS 중심 관리가 더 적합해집니다.

실제 결제 기능을 추가할 때는 신청 당시 기록 금액과 결제 완료 금액, 환불을 반영한 순금액을 같은 의미로 사용하지 않습니다. 별도 결제 구조와 지표 정의가 필요합니다.
...
4. 기준 데이터, 결정적 파생 데이터와 AI 생성 결과는 어떻게 다른가요?
5. 파일, 스프레드시트와 DBMS 중심 관리 방식을 선택할 때 확인할 기준을 세 가지 적어 보세요.
6. 신청 당시 기록 금액과 실제 결제 완료 금액을 구분해야 하는 이유는 무엇인가요?
...
4. 기준 데이터는 업무 판단의 근거이고, 결정적 파생 데이터는 동일한 원본·계산식·기준 시점으로 재생성할 수 있습니다. AI 결과는 모델과 실행 조건에 따라 달라질 수 있어 실행 맥락과 승인 결과를 별도로 관리해야 합니다.
5. 데이터 관계, 여러 사용자의 동시 작업, 정확성, 반복 조회, 권한, 이력과 복구 필요성을 기준으로 판단할 수 있습니다.
6. 신청 당시 기록 금액은 신청 시점의 사실이며, 결제 성공·실패·환불이 반영된 회계 금액과 같다고 단정할 수 없습니다.

15.2의 AI 답변은 현재 데이터 양만 보고 결론을 내렸다는 문제가 있습니다. 온라인 강의 서비스는 학생·강의·수강신청 관계, 여러 운영자의 동시 작업, 상태 일관성, 개인정보 권한과 변경 이력을 함께 고려해야 합니다. 초기 정적 강의 목록은 파일로 관리할 수 있지만 실제 신청을 운영한다면 관계형 데이터베이스가 더 적합합니다.
...
5. 저장 방식은 데이터 양만이 아니라 관계, 동시성, 반복 조회, 권한과 복구 요구사항으로 선택합니다.
6. 기준 데이터, 결정적 파생 데이터와 AI 생성 결과는 의미와 재현 방식이 다릅니다.
7. 이 책에서는 AI가 만든 초안을 PostgreSQL에서 직접 실행하고 기준 데이터와 비교해 검증합니다.
...
현재 가격과 합의 금액이 달라도 할인·가격 변경일 수 있습니다. 샘플에서는 한 행이 다릅니다.

반면 P13-D06의 단순 예제에서는 합의 금액과 결제 기록 금액이 같아야 합니다. 부분 결제·부분 환불을 지원한다면 결제 원장과 환불 금액 모델을 새로 설계해야 합니다.
...
현재 가격과 합의 금액 차이는 정보용 1행입니다. 결과가 있어도 자동 오류가 아닙니다.
...
07과 08을 다른 세션에서 실행했다면 07의 자체 `27/27` 통과 메시지를 별도 실행 증거로 기록합니다.
...
6. `courses.price`, `enrollments.agreed_amount`, `payments.amount`의 의미를 설명해 보세요.
